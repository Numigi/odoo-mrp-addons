    ...
    def _split_productions(self, amounts=False, cancel_remaning_qty=False, set_consumed_qty=False):
        ...
        for production in self:
            ...
            backorder_vals = production.copy_data(
                default=production._get_backorder_mo_vals())[0]
            backorder_qtys = amounts[production][1:]

            next_seq = max(production.procurement_group_id.mrp_production_ids.mapped(
                "backorder_sequence"), default=1)
            ...
            for qty_to_backorder in backorder_qtys:
                ...
                backorder_vals_list.append(dict(
                    ...
                    name=production._get_name_backorder(
                        production.name, next_seq),
                    backorder_sequence=next_seq,
                    state='confirmed'
                ))

        backorders = self.env['mrp.production'].with_context(
            skip_confirm=True).create(backorder_vals_list)

        index = 0
        production_to_backorders = {}
        production_ids = OrderedSet()
        for production in self:
            number_of_backorder_created = len(amounts.get(
                production, _default_amounts(production))) - 1
            production_backorders = backorders[index:index +
        ...
        for production in self:
            for move in production.move_raw_ids | production.move_finished_ids:
                if move.additional:
                    continue
                unit_factor = move.product_uom_qty / \
                    initial_qty_by_production[production]
                initial_move_vals = move.copy_data(
                    move._get_backorder_move_vals())[0]
                move.with_context(
                    do_not_unreserve=True).product_uom_qty = production.product_qty * unit_factor
        ...
        for initial_move, backorder_moves in move_to_backorder_moves.items():
            ...
            initial_move.move_line_ids.with_context(
                bypass_reservation_update=True).write({'product_uom_qty': 0})
                ...
                while float_compare(quantity, 0, precision_rounding=product_uom.rounding) > 0 and move:
                    ...
                    if move == initial_move:
                        move_line.with_context(
                            bypass_reservation_update=True).product_uom_qty = taken_qty_uom
                        if set_consumed_qty:
        ...
        self.env['stock.move.line'].browse(
            move_lines_to_unlink).write({'move_id': False})
        ...
        for production in self:
            ...
            for workorder in (production | bo).workorder_ids:
                workorder.duration_expected = workorder.duration_expected * \
                    workorder.production_id.product_qty / initial_qty
            ...
            for index, workorder in enumerate(bo.workorder_ids):
                remaining_qty = initial_workorder_remaining_qty[index //
                                                                workorders_len]
                if remaining_qty:
